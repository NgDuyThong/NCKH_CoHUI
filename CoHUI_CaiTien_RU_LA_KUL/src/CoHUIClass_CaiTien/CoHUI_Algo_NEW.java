...
//					Tinh Project DB
	...
	{
		// X: itemset hien tai
		// dbProjectX: projected database cua X
		for(int i = k; i < Ikeep.size(); i++)
		{
			...
			int uLA = uX + ruX;
			float phanso = (kulcX * (float)k / (float)(supportX)) 
					+ 1.0f / (float) Support_Ikeep.get(itemRight - 1);
			...
			for(Transaction tran : dbProjectX.getTransactions())
			{
				...
				// itemRight khong xuat hien trong projected transaction nay
			...
			if(kulcX_ >= minCor)
			{
				if(uX_ >= minUtil)
				{
					ItemSet itemsetCoHUI = new ItemSet(X_, uX_, kulcX_);
					CoHUIs.addItemset(itemsetCoHUI, k + 1);
				}

				// Chi khi ung vien con kha nang mo rong moi build projected DB
				if(uX_ + ruX_ >= minUtil)
				{
					Dataset dbProject = buildProjectedDatabase(dbProjectX, itemRight);
	...
	private Dataset buildProjectedDatabase(Dataset dbProjectX, int itemRight)
	{
		Dataset dbProject = new Dataset();
		...
		for(Transaction tran : dbProjectX.getTransactions())
		{
			...
			// itemRight xuat hien trong transaction
			ru_temp -= tran.utilities[j];

			Transaction newtran = createProjectedTransaction(tran, j, ru_temp);
			dbProject.add(newtran);
		}
	...
	private Transaction createProjectedTransaction(Transaction tran, int position, int ruAfterPosition)
	{
		Transaction newtran = new Transaction(
				Arrays.copyOfRange(tran.items, position + 1, tran.items.length),
				Arrays.copyOfRange(tran.utilities, position + 1, tran.items.length),
				ruAfterPosition
		);

		newtran.Uprefix = tran.Uprefix + tran.utilities[position];
		return newtran;
	}
...
//	}
	...
	public ItemSets getCoHUIs() { return CoHUIs; }
	public int[] getNewNamesToOldNames() { return newNamesToOldNames; }
